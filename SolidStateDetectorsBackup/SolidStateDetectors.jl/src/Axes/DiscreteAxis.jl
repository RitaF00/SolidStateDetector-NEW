...
# serve principalmente pe evitare problemi con il calcolo dell'angolo, ma a sto punto lo facciamo per tutti.
function even_tick_axis(ax::DiscreteAxis)
    # garantisce un numero pari di tick in ogni asse
    if isodd(length(ax))
        # se l'asse ha un numero DISPARI se punti, ne aggiunge uno dove trova la distanza maggiore
        # in questoo modo la griglia rimane il più uniforme possibile
        int = ax.interval
        ticks = ax.ticks
