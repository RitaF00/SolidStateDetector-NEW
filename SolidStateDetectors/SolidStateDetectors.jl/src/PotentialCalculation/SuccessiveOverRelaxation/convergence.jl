function _update_till_convergence!(pcs::PotentialCalculationSetup{T,S,3},
    ...
    while c > c_limit
        ...
        if c <= c_limit
            ...
            println("🟢 STOP: convergenza raggiunta (c <= c_limit).")
            break
        end
    ...
    if stop_reason === nothing
        if c <= c_limit
            stop_reason = :converged
            println("🟢 STOP: convergenza raggiunta (fuori dal ciclo, c <= c_limit).")
        elseif max_n_iterations != -1 && n_performed_iterations >= max_n_iterations
            ...
            stop_reason = :unknown
            println("ℹ️ STOP: ciclo terminato (reason unknown). c = $c")
        end
    end
