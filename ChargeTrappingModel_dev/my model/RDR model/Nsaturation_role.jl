...
function generate_Li_cells(Lx, Ly, Lz, dx, α, Ns, D_Li, t_ann, x_saturation, Nd_saturation)
    ...
    for xi in x_slices
        ...
        N_i = rand(Poisson(λ))
        # faccio il print 'secco' per togliere il rumore statistico
        #N_i = round(Int, λ)
        total_Li += N_i
...
colors = colors = [:mediumpurple2, :deeppink, :deepskyblue, :orange]
...
for (k, Nd_saturation) in enumerate(Nd_sat_list)
    ...
    println("Nd_saturation = ", Nd_saturation)
    ...
    # =====================
    # NORMALIZATION + STATS (like code 2)
    # =====================
    ...
    CCE_all[Nd_saturation] = (CCE_mean, CCE_std)
end
...
for (k, Nd_saturation) in enumerate(Nd_sat_list)
    ...
    if haskey(CCE_all, Nd_saturation)

        CCE_mean, CCE_std = CCE_all[Nd_saturation]
        ...
        plot!(
            ...
            lw=2,
            color=colors[k],
            label="Nd_sat = 1e$(Int(log10(Nd_saturation)))",
            ribbon=CCE_std,
            fillalpha=0.25
        )
    end
...
display(p)
savefig(p, "plot/CCE_vs_Ndsat.png")



# =====================
# OPTIONAL: PRINT PRECIPITATES
print("============================================================")
for Nd_saturation in sort(collect(keys(precip_counts)))

    exp = Int(round(log10(Nd_saturation)))

    println("Nd_sat = 1e", exp,
        " → precipitates = ",
        precip_counts[Nd_saturation])
end# =====================
...
# DATA
# =====================
Nd_list = sort(collect(keys(precip_counts)))

depth_mm = Float64[]
precip_vals = Float64[]
Nd_used = Float64[]

for Nd_saturation in Nd_list

    depth_list = 0:dx:0.11
    Nd_profile = Ns .* erfc.(depth_list ./ (2 * sqrt(D_Li * t_ann)))

    diff_vals = Nd_profile .- Nd_saturation
    idx = findfirst(i -> diff_vals[i] <= 0, eachindex(diff_vals))
    ...
    push!(depth_mm, depth_list[idx] * 10)
    push!(precip_vals, precip_counts[Nd_saturation])
    push!(Nd_used, Nd_saturation)
end

perm = sortperm(depth_mm)
depth_mm = depth_mm[perm]
precip_vals = precip_vals[perm]
Nd_used = Nd_used[perm]

x = 1:length(depth_mm)

bottom_labels = string.(round.(depth_mm, digits=3))
top_labels = ["1e$(string(round(log10(v), digits=3)) )" for v in Nd_used]
...
p = plot(
    ...
    lw=2,
    marker=:circle,
    xticks=(x, bottom_labels),
    xlabel="Saturation depth (mm)",
    ylabel="Total precipitates",
    legend=false,
    frame=:box,
    grid=:dash,
    gridalpha=0.15,
...
# TOP labels come overlay (NON secondo plot)
annotate!(
    p,
    [(x[i], maximum(precip_vals) * 1.05, text(top_labels[i], 8, :center))
     for i in eachindex(x)]
)
annotate!(
    p,
    (x[7], maximum(precip_vals) * 1.15, text("Saturation concentration"))
)
plot!(
    p,
